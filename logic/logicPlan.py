...
def sentence1() -> Expr:
    ...
    firstState= (a | b)
    secondState = ~a % (~b | c)
    thirdState =  logic.disjoin(~a, ~b, c)
    return conjoin(firstState, secondState, thirdState)
...
def sentence2() -> Expr:
    ...
    thirdState = ~(b & ~c) >> a
    fourthState = ~d >> c
    return conjoin(firstState, secondState, thirdState, fourthState)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    a, b, c, d = PropSymbolExpr("PacmanAlive_1"), PropSymbolExpr("PacmanAlive_0"), PropSymbolExpr("PacmanBorn_0"), PropSymbolExpr("PacmanKilled_0")
    firstState = a % ((b & ~d) |  (~b & c))
    secondState = ~(b & c)
    return conjoin(firstState, secondState, c)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    coords = list(itertools.combinations(literals, 2))
    nextLiterals = []
    for a, b in coords:
            nextLiterals.append(disjoin(~a, ~b))
    return conjoin(nextLiterals)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return PropSymbolExpr(pacman_str, x, y, time=time) % atLeastOne(possible_causes)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        ...
        notPossibleMoves.append(conjoin(a >> b))
    for x, y in non_outer_wall_coords:
        possibleMoves.append(PropSymbolExpr(pacman_str, x, y, time=t))
    for element in DIRECTIONS:
        moves.append(PropSymbolExpr(element, time=t))
    if sensorModel != None:
        axiomsResult = sensorModel(t, non_outer_wall_coords)
        if axiomsResult != None:
            pacphysics_sentences.append(axiomsResult)
    if successorAxioms != None and t != 0:
        succResult = successorAxioms(t, walls_grid, non_outer_wall_coords)
        if succResult != None:
            pacphysics_sentences.append(succResult)
    pacphysics_sentences.extend(notPossibleMoves)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    a = conjoin(KB)
    b = PropSymbolExpr(pacman_str, x1, y1, time=1)
    model1 = findModel(a & b)
    model2 = findModel(a & ~b)
...
def positionLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for t in range(50):
        possibleMoves, possibleStates = [], []
        for x, y in non_wall_coords:
            possibleMoves.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(possibleMoves))
        a = conjoin(KB)
        b = PropSymbolExpr(pacman_str, xg, yg, time=t)
        if findModel(a & b):
            return extractActionSequence(findModel(a & b), DIRECTIONS)
        actions = []
        for element in DIRECTIONS:
            actions.append(PropSymbolExpr(element, time=t))
        KB.append(exactlyOne(actions))
        for x, y in non_wall_coords:
            possibleStates.append(pacmanSuccessorAxiomSingle(x, y, time=t + 1, walls_grid=walls_grid))
        KB.append(conjoin(possibleStates))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    foodPos = []
    for x, y in food:
        foodPos.append(PropSymbolExpr(food_str, x, y, time=0))
    KB.append(conjoin(foodPos))
    for t in range(50):
        possibleMoves, possibleStates, noFoodPos = [], [], []
        for x, y in non_wall_coords:
            possibleMoves.append(PropSymbolExpr(pacman_str, x, y, time=t))
        for x, y in food:
            noFoodPos.append(~PropSymbolExpr(food_str, x, y, time=t))
        KB.append(exactlyOne(possibleMoves))
        a = conjoin(KB)
        b = conjoin(noFoodPos)
        if findModel(a & b):
            return extractActionSequence(findModel(a & b), DIRECTIONS)
        actions = [PropSymbolExpr(element, time=t) for element in DIRECTIONS]
        KB.append(exactlyOne(actions))
        models = []
        for x, y in non_wall_coords:
            possibleStates.append(pacmanSuccessorAxiomSingle(x, y, time=t+1, walls_grid=walls))
            pacmanT = PropSymbolExpr(pacman_str, x, y, time=t)
            foodT = PropSymbolExpr(food_str, x, y, time=t)
            foodTPlus = PropSymbolExpr(food_str, x, y, time=(t + 1))
            models.append((foodT & ~pacmanT) % foodTPlus)
        KB.append(conjoin(possibleStates))
        KB.append(conjoin(models))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for x, y in all_coords:
        if (x, y) in walls_list:
            wallsExpr.append(PropSymbolExpr(wall_str, x, y))
        else:
            wallsExpr.append(~PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(wallsExpr))
    for t in range(agent.num_timesteps):
        ...
        possibleLoc = []
        for x, y in non_outer_wall_coords:
            entailOne = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t))
            entailTwo = entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t))
            if entailOne:
                KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
            if entailTwo:
                KB.append(~PropSymbolExpr(pacman_str, x, y, time=t))
            if (not entailTwo):
                possibleLoc.append((x, y))
        possible_locations = possibleLoc
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for x, y in non_outer_wall_coords:
            entailOne = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            entailTwo = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))
            if entailOne:
                known_map[x][y] = 1
            ...
            if entailTwo:
                known_map[x][y] = 0
                KB.append(~PropSymbolExpr(wall_str, x, y))
            if (not entailTwo) and (not entailOne):
                known_map[x][y] = -1
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map
