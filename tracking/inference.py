    ...
    for var in variables:
        ...
        for x in range(0, X_RANGE):
            for y in range(0, Y_RANGE):
                givenPos = (x, y)
                obsDist = manhattanDistance(givenPos, (X_RANGE, Y_RANGE))
                trueDist = manhattanDistance(givenPos, (X_RANGE, Y_RANGE))
                posManDist = manhattanDistance(givenPos, (X_RANGE, Y_RANGE)) + MAX_NOISE
                posManDistMargin = manhattanDistance(givenPos, (X_RANGE, Y_RANGE)) - MAX_NOISE
                if obsDist > 0 and trueDist > 0:
                    if var not in [OBS0, OBS1]:
                        variableDomainsDict[var] += [givenPos]
    for var in variables:
        if var in [OBS0, OBS1]:
            for x in range(0, X_RANGE + Y_RANGE + 6):
                variableDomainsDict[var] += [x]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors, joinFactor = joinFactorsByVariable(factors, var)
            if len(joinFactor.unconditionedVariables()) > 1:
                elimVar = eliminate(joinFactor, var)
                factors.append(elimVar)
        joinFactor = joinFactors(factors)
        return normalize(joinFactor)
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        t = self.total()
        assignRandom = random.Random().uniform(0, t)
        prob = 0
        for key, value in self.items():
            if assignRandom < prob + value:
                return key
            prob += value
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            self.beliefs[ghostPos] = self.getObservationProb(observation, pacPos, ghostPos, jailPos) * self.beliefs[ghostPos]
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        pacPos = gameState.getPacmanPosition()
        bPrime = DiscreteDistribution()

        for pos in self.allPositions:
            posDist = self.getPositionDistribution(gameState, pos)
            for key, value in posDist.items():
                bPrime[key] += value * self.beliefs[pos]
        self.beliefs = bPrime
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        numParticles = self.numParticles / len(self.legalPositions)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            dist[particle] = self.particles.count(particle)
        dist.normalize()
        return dist
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        dist = DiscreteDistribution()
        for particle in self.particles:
            dist[particle] += self.getObservationProb(observation, pacPos, particle, jailPos)
        dist.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        pacPos = gameState.getPacmanPosition()
        dist = DiscreteDistribution()
        distBelief = self.getBeliefDistribution()
        for pos in self.legalPositions:
            posDist = self.getPositionDistribution(gameState, pos)
            for nextPos, value in posDist.items():
                dist[nextPos] += value * distBelief[pos]
