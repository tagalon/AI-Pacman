...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        bestPos = None
        minMazeDistance = None
        for dist in livingGhostPositionDistributions:
            likelyPos = dist.argMax()
            calcDist = self.distancer.getDistance(pacmanPosition, likelyPos)
            if (minMazeDistance is None and bestPos is None) or minMazeDistance > calcDist:
                minMazeDistance = calcDist
                bestPos = likelyPos
        bestAction = None
        bestActDist = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            actDist = self.distancer.getDistance(successorPosition, bestPos)
            if (bestActDist is None and bestAction is None) or bestActDist > actDist:
                bestActDist= actDist
                bestAction = action
        return bestAction
        "*** END YOUR CODE HERE ***"
